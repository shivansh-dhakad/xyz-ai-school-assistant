...
function switchView(view) {
  ...
  if (view === "chat" && !state.chatSessionId) startChat();
}
...
async function renderStudentHome(el) {
  ...
  el.innerHTML = `<div class="student-dashboard">
    ...
    <section class="student-dashboard-card student-overview"><h2>Attendance overview</h2><div class="student-divider"></div><div class="student-ring-wrap">${ringSvg(summary.percentage, 230, 20)}<strong>${summary.percentage}%</strong><span>Present rate</span></div><div class="student-totals"><div><span>Classes</span><b>${summary.total_days}</b></div><div><span>Attended</span><b>${summary.present + summary.late}</b></div><div class="missed"><span>Missed</span><b>${summary.absent}</b></div></div></section>
    <section class="student-dashboard-card student-contact-card"><span>Need help with your attendance or classes?</span><h2>Contact your teacher</h2><p>Send a contact request directly to your assigned teacher.</p><button class="dashboard-primary-btn" onclick="requestContact(${summary.student_id}, 'teacher')">Contact teacher</button></section>
    <section class="student-dashboard-card student-recent"><h2>Recent attendance</h2><div class="student-divider"></div>${history.records.slice(0, 5).map((record) => `<div class="student-recent-row"><i class="${record.status}"></i><div><strong>${record.date}</strong><small>${record.status === 'absent' ? 'Attendance not recorded' : 'Attendance recorded'}</small></div><span class="student-status ${record.status}">${t(record.status, state.lang)}</span></div>`).join("")}</section>
  </div>`;
  return;
  const legacySummary = await api(`/student/${await currentStudentId()}/attendance`);
  const legacyHistory = await api(`/student/${legacySummary.student_id}/attendance/history?period=last_30_days`);
  el.innerHTML = `
...
async function requestContact(studentId, target) {
  try {
    await api("/contact-requests", { method: "POST", body: { student_id: studentId, target, reason: "Student requested contact via the portal" } });
    toast("Contact request sent to your teacher.");
  } catch (e) {
    toast(e.message);
  }
}
...
async function renderParentHome(el) {
  ...
  el.innerHTML = `<div class="section-title">${t("myChildren", state.lang)}</div>` + cards.join("") +
    `<button class="chip-btn contact-principal-btn" onclick="escalate(${data.children[0].student_id}, 'management')">${t("contactPrincipal", state.lang)}</button>`;
}
...
async function renderTeacherHome(el) {
  ...
  el.innerHTML = `<div class="section-title">${t("myClasses", state.lang)}</div>` +
    data.classes.map(c => `
...
async function renderPrincipalHome(el) {
  ...
  el.innerHTML = `
    ...
    <div class="stat-card">
      ${data.by_class.map(c => `
      <div class="class-card" onclick="openPrincipalClass(${c.class_id}, '${c.class_name}')">
        <div class="avatar-chip">${c.class_name.slice(0,2)}</div>
        <div class="info">
